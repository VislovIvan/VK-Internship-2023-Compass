...
//
//  Created by Ivan Vislov on 08.05.2023.
//
...
struct ContentView: View {
    ...
    var title: some View {
        VStack(spacing: 5) {
            Text("\(String(format: "%.0f", abs(compassHeading.degrees)))º \(Direction(compassHeading.degrees))".uppercased())
                .font(.largeTitle)
            Text(locationManager.city.uppercased())
                .font(.headline)
                .fontWeight(.medium)
    ...
    var circleLabel: some View {
        CircleLabelView(radius: 120, text: locationText.uppercased(), kerning: 3, size: CGSize(width: 225, height: 225))
    ...
    var locationText: String {
        if let myLocation = locationManager.location {
            return "Широта: \(myLocation.latitude.formatted(.number.precision(.fractionLength(4)))),\n Долгота: \(myLocation.longitude.formatted(.number.precision(.fractionLength(4)))),\n Высота над уровнем моря: \(locationManager.altitude?.formatted(.number.precision(.fractionLength(2))) ?? "недоступна") м"
        } else {
